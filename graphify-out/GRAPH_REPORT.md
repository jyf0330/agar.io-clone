...
## Corpus Check
- 110 files · ~98,966 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 508 nodes · 749 edges · 28 communities detected
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 121 edges (avg confidence: 0.8)
...
1. `Orchestrator` - 26 edges
2. `ask()` - 16 edges
3. `ChatClient` - 16 edges
4. `Canvas` - 13 edges
5. `addPlayer()` - 12 edges
6. `emitIfReady()` - 12 edges
...
Cohesion: 0.1
Nodes (12): clamp(), randomWalkIntent(), estimatePromptTokens(), getChatFallbackReply(), getMockChatReply(), isFollowPlayerMessage(), Orchestrator, parseJsonPayload() (+4 more)
...
Cohesion: 0.09
Nodes (16): getPosition(), validNick(), addNew(), Food, removeExcess(), balanceMass(), enumerateVisibleWorld(), getVisibleWorldForPlayer() (+8 more)
...
- **Thin community `Community 16`** (7 nodes): `game-logic.js`, `massFood.js`, `adjustForBoundaries()`, `addNew()`, `constructor()`, `move()`, `remove()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (5 nodes): `player-card-scale.js`, `canScaleIn()`, `canScaleOut()`, `clampScale()`, `getNextScale()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (4 nodes): `clearModule()`, `createOpenAiMockServer()`, `loadWrapperFixture()`, `llm-wrapper.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `addPlayer()` connect `Community 6` to `Community 8`, `Community 1`, `Community 3`?**
  _High betweenness centrality (0.190) - this node is a cross-community bridge._
- **Why does `bootstrapNpc()` connect `Community 8` to `Community 6`?**
  _High betweenness centrality (0.171) - this node is a cross-community bridge._
- **Why does `Orchestrator` connect `Community 0` to `Community 8`?**
  _High betweenness centrality (0.169) - this node is a cross-community bridge._
- **Are the 7 inferred relationships involving `ask()` (e.g. with `createCacheKey()` and `get()`) actually correct?**
  _`ask()` has 7 INFERRED edges - model-reasoned connections that need verification._
