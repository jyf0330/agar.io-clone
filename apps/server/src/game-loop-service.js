...
function createGameLoopService(options) {
    ...
    const spectatorUpdateIntervalMs = typeof syncConfig.spectatorUpdateIntervalMs === 'number'
        ...
        : 100;
    const botMovementUpdateIntervalMs = typeof syncConfig.botMovementUpdateIntervalMs === 'number'
        && syncConfig.botMovementUpdateIntervalMs > 0
        ? syncConfig.botMovementUpdateIntervalMs
        : 0;
    const syncMetrics = {
    ...
    function tickGame() {
        ...
        map.players.handleCollisions(function (gotEaten, eater) {
            ...
            if ((typeof playerGotEaten.isInvincible === 'function' && playerGotEaten.isInvincible(collisionNow))
                || (typeof eaterPlayer.isInvincible === 'function' && eaterPlayer.isInvincible(collisionNow))) {
                return;
            }
    ...
    function updateSpectator(socketId, now) {
        ...
        socket.emit(
            ...
            spectatorData,
            projectPlayersMovementForSync(map.players.data),
            map.food.data,
            map.massFood.data,
            map.viruses.data,
            map.partLoot.data,
    ...
    function recordSyncMetrics(startedAt, packetCount) {
        ...
        console.log('[SYNC] batches=' + syncMetrics.batches
            + ' packets=' + syncMetrics.packets
            + ' avgMs=' + (syncMetrics.totalMs / syncMetrics.batches).toFixed(2)
            + ' maxMs=' + syncMetrics.maxMs);
