...
const orchestrator = new Orchestrator({
    ...
    paintPlayer(targetPlayer, npc) {
        ...
        targetPlayer.playerCardPreviewDataUrl = createPaintedAvatarDataUrl(
            targetPlayer.playerCardPreviewDataUrl,
            npc.color,
            targetPlayer.npcPaintCount
        );
...
const PET_CATALOG = {
    mochi: {petId: 'mochi', npcId: 'mochi', name: 'Mochi', personality: '安静诗意'},
    doudou: {petId: 'doudou', npcId: 'doudou', name: 'Doudou', personality: '调皮捣蛋'},
    wugui: {petId: 'wugui', npcId: 'wugui', name: 'Wugui', personality: '老成稳重'}
};
const PET_ALIASES = {
    mochi: 'mochi',
    '麻薯': 'mochi',
...
function speakPreviousExpectations(currentPlayer) {
    const activeNpc = activePet.getActiveNpc(npcRoster, currentPlayer, {fallbackToFirst: false});
    if (!activeNpc) {
        return;
    }
...
function refreshNpcRelationshipsForPlayers() {
    ...
    const now = Date.now();
    const ttlMs = config.npc && typeof config.npc.relationshipCacheTtlMs === 'number'
        ? config.npc.relationshipCacheTtlMs
        : 10000;
...
if (npcFeaturesEnabled) {
    ...
    setInterval(
        refreshNpcRelationshipsForPlayers,
        config.npc && config.npc.relationshipRefreshIntervalMs ? config.npc.relationshipRefreshIntervalMs : 10000
    );
}
setInterval(
    gameLoopService.sendMetaUpdates,
    config.sync && config.sync.playerMetaUpdateIntervalMs ? config.sync.playerMetaUpdateIntervalMs : 2000
);
