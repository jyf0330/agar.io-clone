...
describe('util.js', () => {
  ...
  describe('removeIndexes', () => {
    it('should remove the requested indexes without mutating the input array', () => {
      const input = [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }];
      const output = util.removeIndexes(input, [1, 3]);

      expect(output).to.deep.equal([{ id: 1 }, { id: 3 }]);
      expect(input).to.deep.equal([{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }]);
    });
  });
});
